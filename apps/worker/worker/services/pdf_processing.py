...
def extract_pages(
    ...
    with fitz.open(path) as document:
        for index, page in enumerate(document):
            ...
            ocr_text = normalize_text(pytesseract.image_to_string(image, lang=ocr_language))
            confidence = 0.75 if len(ocr_text) >= min_native_chars else 0.3
            pages.append(ExtractedPage(index + 1, ocr_text, "ocr", confidence))
    return pages
